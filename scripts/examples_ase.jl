...
# Activate local project so we use the in-tree FreeBird version with the latest code
import Pkg
Pkg.activate(joinpath(@__DIR__, ".."))
Pkg.instantiate()
Pkg.add("PythonCall")
ENV["JULIA_PYTHONCALL_EXE"] = "your_environment/bin/python"
...
ase_pot = ASELennardJones(epsilon = 0.1, sigma = 2.5, cutoff = 4.0)
...
# Create a `LJAtomWalkers` object that uses the ASE-based potential.
ls = LJAtomWalkers(walkers, ase_pot)
nothing #hide
...
# Run the Monte Carlo simulation with ASE potential
mc_energies, mc_ls, mc_cvs, acceptance_rates = monte_carlo_sampling(at, ase_pot, mc_params)
...
wl_params = WangLandauParameters(num_steps=10_000,
                            ...
                            max_iter=10000,
                            f_min=1.00001)
# Viola! We can now run the Wang-Landau simulation with ASE potential.
energies_wl, configs, wl_params, S, H = wang_landau(at, ase_pot, wl_params)
